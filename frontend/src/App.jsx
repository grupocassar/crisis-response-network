...
const zoneUrgencyLabels = {
  alta: 'Vidas en Peligro (Atrapados)',
  media: 'Familias Aisladas / Sin Salida',
  baja: 'Zona Afectada (Sin heridos)'
};
...
export default function App() {
  ...
  const handleSubmitAporteZona = async (e) => {
    ...
    try {
      ...
      setSelectedItem(updatedItem);
      showNotification('Información de foco de rescate actualizada');
      setView('detail');
    } catch (err) {
    ...
    <div className="flex flex-col h-full gap-4 animate-fade-in">
      <div className="bg-black text-white p-6 pb-8">
        <div className="flex justify-between items-start">
          <h2 className="text-3xl font-black uppercase tracking-tight mb-2 leading-none">Sistema de<br />Rescate Humano</h2>
      ...
      <div className="px-4 flex flex-col gap-4 -mt-6">
        <button onClick={() => { setView('personas'); setActiveTab('personas'); }} className="bg-white p-6 border-4 border-black hover:bg-gray-50 flex flex-col items-start gap-2 transition-transform active:scale-[0.98]">
          ...
          <p className="text-left text-sm text-gray-600 font-medium">Buscar personas desaparecidas, heridas o localizadas.</p>
        </button>

        <button onClick={() => { setView('zonas'); setActiveTab('zonas'); }} className="bg-red-600 text-white p-6 border-4 border-black hover:bg-red-700 flex flex-col items-start gap-2 transition-transform active:scale-[0.98]">
          <Map size={32} className="mb-2" />
          <div className="flex justify-between w-full items-center">
            <h3 className="text-2xl font-black uppercase">Focos de Rescate (Colapsos/Atrapados)</h3>
            <span className="bg-white text-red-600 text-xs px-2 py-1 font-black">{zonas.length} regs</span>
          </div>
          <p className="text-left text-sm text-red-100 font-medium">Reportar edificios colapsados, atrapados o familias aisladas.</p>
        </button>
      </div>
    </div>
  );
        ...
        <div className="bg-black p-4 sticky top-0 z-10 flex flex-col gap-3 shadow-lg">
          <div className="flex justify-between items-center text-white">
            <h2 className="text-xl font-black uppercase flex items-center gap-2">
              {isPersonas ? <User size={20} /> : <Map size={20} />} {isPersonas ? 'Personas' : 'Focos de Rescate'}
            </h2>
          ...
          <div className="flex gap-2">
            <input
              type="text"
              placeholder={isPersonas ? 'Buscar nombre (Búsqueda difusa)...' : 'Buscar sector o foco...'}
            ...
            <>
              {filtered.map(item => (
                <div key={item.id} onClick={() => {
                  ...
                  setActiveTab(isPersonas ? 'personas' : 'zonas');
                  setView('detail');
                }} className="bg-white border-2 border-black p-4 cursor-pointer active:scale-[0.98] transition-transform">
                  <div className="flex justify-between items-start mb-3">
                    <TrustBadge level={item.trust_level} />
                    {isPersonas && <StatusPill status={item.status} />}
                    {!isPersonas && <span className={`text-xs px-2 py-1 font-bold uppercase text-white ${item.urgency === 'alta' ? 'bg-red-600' : 'bg-orange-500'}`}>{zoneUrgencyLabels[item.urgency] || item.urgency}</span>}
                  </div>
  ...
  const renderFormZonaView = () => (
    <div className="bg-white min-h-screen animate-fade-in">
      ...
      <form onSubmit={handleSubmitZona} className="p-4 space-y-5">
        <div>
          <label className="block text-sm font-black uppercase mb-2">1. Nombre del Sector / Foco *</label>
        ...
        <div>
          <label className="block text-sm font-black uppercase mb-2">2. Riesgo de Vida *</label>
        ...
        <div>
          <label className="block text-sm font-black uppercase mb-2">3. Situación (Describa el rescate) *</label>
    ...
    return (
      <div className="bg-white min-h-screen animate-fade-in">
        ...
        <form onSubmit={handleSubmitAportePersona} className="p-4 space-y-5">
          <div>
            <label className="block text-sm font-black uppercase mb-2">1. Estado vital *</label>
            <div className="grid grid-cols-2 gap-2">
              {['buscado', 'a_salvo', 'herido', 'fallecido'].map(s => (
          ...
          <div>
            <label className="block text-sm font-black uppercase mb-2">2. Ubicación actual *</label>
  ...
  const renderFormAporteZonaView = () => {
    ...
    return (
      <div className="bg-white min-h-screen animate-fade-in">
        <div className="bg-red-600 text-white p-4 sticky top-0 flex justify-between items-center shadow-md">
          <h2 className="font-black uppercase">Aportar Info - Foco de Rescate</h2>
        ...
        <form onSubmit={handleSubmitAporteZona} className="p-4 space-y-5">
          <div>
            <label className="block text-sm font-black uppercase mb-2">1. Riesgo de vida *</label>
            <div className="grid grid-cols-3 gap-2">
              {['alta', 'media', 'baja'].map(u => (
          ...
          <div>
            <label className="block text-sm font-black uppercase mb-2">2. Nueva situación *</label>
  ...
  return (
    <div className="max-w-md mx-auto bg-gray-100 min-h-screen font-sans relative overflow-hidden">
      ...
      {view === 'personas' && renderDashboardView()}
      {view === 'zonas' && renderDashboardView()}
      {view === 'detail' && renderDetailView()}
      {view === 'form_persona' && renderFormPersonaView()}
      {view === 'form_zona' && renderFormZonaView()}
